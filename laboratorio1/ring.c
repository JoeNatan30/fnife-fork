...
// Inicio de los forks
	...
	for(i=1; i< nprocs; i++) { /*create the remaining processes */
		...
		//En caso sea  el padre
		if(childpid > 0) /* for the parent process, reassign stdout */
		...
		if(childpid) break;//Si es el padre, entonces sale
	}

	fprintf(stderr, "This is process %d with %ld and parent id %ld\n", i, (long)getpid(), (long)getppid());
	return 0;
}
